...
module.exports = (t) => {
	...
	t.describe('with options.serializers', (t1) => {
		...
		t1.before((done) => {
			...
			Object.keys(thisSerializers).forEach((key) => {
				sinon.spy(thisSerializers, key);
			});

			logger = createLogger({
				serializers: thisSerializers
			});
		...
		t1.it('has default serializer', () => {
			assert.isEqual(3, Object.keys(logger.serializers).length);
			assert.isEqual('function', typeof logger.serializers.err);

			assert.isEqual(1, serializers.err.callCount);

			const options = serializers.foo.getCall(0).args[0];
			assert.isEqual('object', typeof options);
			assert.isNotEmpty(options);
		});

		t1.it('has given serializers', () => {
			assert.isEqual(3, Object.keys(logger.serializers).length);
			assert.isEqual('function', typeof logger.serializers.foo);
			assert.isEqual('function', typeof logger.serializers.baz);
			assert.isEqual(1, serializers.foo.callCount);
			assert.isEqual(1, serializers.baz.callCount);

			let options;

			options = serializers.foo.getCall(0).args[0];
			assert.isEqual('object', typeof options);
			assert.isNotEmpty(options);

			options = serializers.baz.getCall(0).args[0];
			assert.isEqual('object', typeof options);
			assert.isNotEmpty(options);
		});
	});
